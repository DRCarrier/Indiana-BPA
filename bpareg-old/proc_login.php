...
<body>
    ...
    $school_number = $_POST['school_number']; // fixed: use correct variable name
    ...
    $stmt2->bindParam(':password3', $hashed_password); // fixed: bind hashed password instead of 'password'
    $stmt2->execute();
    $login = $stmt2->fetch(PDO::FETCH_ASSOC);

    // check if login details are correct
    if ($login && $login['username'] == $username && password_verify($password3, $login['password3'])) { // fixed: verify password using password_verify
        //Creates a session variable
        session_start();
        $_SESSION["school_number"]= $school_number; // fixed: use correct variable name
...
$login = $stmt2->fetch(PDO::FETCH_ASSOC);
    ...
    // check if login details are correct
    if ($login && $login['username'] == $username && $login['password3'] == $hashed_password) {
        //Creates a session variable
    session_start();
